...
import { collection, onSnapshot, doc, setDoc, updateDoc, addDoc, serverTimestamp, query, orderBy } from 'firebase/firestore';
...
export default function GuardianDashboard() {
    ...
    const unsubSubjects = onSnapshot(collection(db, 'subjects'), (snapshot) => {
      if (snapshot.empty) {
        ...
        const defaultSubjects = [
          { name: 'HINDI 1', progress: 0, chapters: 0, completed: 0, order: 1, chapterList: [] },
          { name: 'HINDI 2', progress: 0, chapters: 0, completed: 0, order: 2, chapterList: [] },
          { name: 'ENG 1', progress: 0, chapters: 0, completed: 0, order: 3, chapterList: [] },
          { name: 'ENG 2', progress: 0, chapters: 0, completed: 0, order: 4, chapterList: [] },
          { name: 'MATHS', progress: 0, chapters: 0, completed: 0, order: 5, chapterList: [] },
          { name: 'GK', progress: 0, chapters: 0, completed: 0, order: 6, chapterList: [] },
          { name: 'HISTORY', progress: 0, chapters: 0, completed: 0, order: 7, chapterList: [] },
          { name: 'CIVICS', progress: 0, chapters: 0, completed: 0, order: 8, chapterList: [] },
          { name: 'GEOGRAPHY', progress: 0, chapters: 0, completed: 0, order: 9, chapterList: [] },
          { name: 'PHYSICS', progress: 0, chapters: 0, completed: 0, order: 10, chapterList: [] },
          { name: 'CHEMISTRY', progress: 0, chapters: 0, completed: 0, order: 11, chapterList: [] },
          { name: 'BIOLOGY', progress: 0, chapters: 0, completed: 0, order: 12, chapterList: [] },
          { name: 'SANSKRIT', progress: 0, chapters: 0, completed: 0, order: 13, chapterList: [] },
        ];
  ...
  const handleAddChapter = async (subjectName, e) => {
    ...
    await addDoc(collection(db, 'activities'), {
      message: `Guardian added chapter "${newChapterInput}" to ${subjectName}`,
  ...
  const handleToggleChapter = async (subjectName, chapterIndex) => {
    ...
    if(updatedList[chapterIndex].isCompleted) {
       await addDoc(collection(db, 'activities'), {
         message: `Guardian marked "${updatedList[chapterIndex].name}" as completed in ${subjectName}`,
         time: new Date().toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'}),
         timestamp: serverTimestamp()
       });
    }
  ...
  const handlePdfUpload = (subjectName, e) => {
    ...
    reader.onloadend = async () => {
      ...
      await addDoc(collection(db, 'activities'), {
        message: `Guardian uploaded syllabus PDF "${file.name}" for ${subjectName}`,
        time: new Date().toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'}),
        timestamp: serverTimestamp()
      });
    };
  ...
  const handleDeleteChapter = async (subjectName, chapterIndex, e) => {
    ...
    await addDoc(collection(db, 'activities'), {
      message: `Guardian removed chapter "${removedChapter.name}" from ${subjectName}`,
      time: new Date().toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'}),
      timestamp: serverTimestamp()
    });
  };
  ...
  const handleScheduleTest = async (e) => {
    ...
    // Add activity
    await addDoc(collection(db, 'activities'), {
      message: `Guardian Scheduled Test: ${scheduleSubject} - ${scheduleTopics}`,
      time: new Date().toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'}),
      timestamp: serverTimestamp()
    });
  ...
  const handleInlineUpload = async (e, testId) => {
    e.preventDefault();
    if (!answerSheetBase64) { alert("Please attach an image first."); return; }
    ...
    if(testToUpdate) {
      await addDoc(collection(db, 'activities'), {
        message: `Answer Sheet Uploaded for ${testToUpdate.subject} Test`,
        time: new Date().toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'}),
        timestamp: serverTimestamp()
      });
    }
  ...
  const avgProgress = subjects.length > 0 
    ? Math.round(subjects.reduce((acc, curr) => acc + curr.progress, 0) / subjects.length)
    : 0;
  ...
  const completedTestsThisMonth = weeklyTests.filter(t => t.status === 'COMPLETED' && (!t.testDate || t.testDate.startsWith(currentMonthPrefix))).length;
      ...
      <main className="main-content">
        <div className="topbar">
          <h1 className="page-title">
            {activeTab === 'dashboard' && 'Guardian Dashboard'}
            {activeTab === 'activity-feed' && 'Activity Feed'}
            {activeTab === 'syllabus-progress' && 'Syllabus Tracker'}
        ...
        {hasTestToday && (
          <div style={{background: '#EEF2FF', border: '1px solid #6366F1', borderLeft: '4px solid #6366F1', padding: '16px', borderRadius: '8px', marginBottom: '24px', display: 'flex', alignItems: 'center', gap: '12px'}}>
            ...
            <div>
              ...
              <p style={{color: '#4F46E5', margin: 0, fontSize: '14px'}}>You have a test scheduled for today. Good luck!</p>
            </div>
          </div>
        )}
        ...
        {activeTab === 'dashboard' && (
          ...
          <div className="card stat-card">
            ...
            <div className="stat-label">Total Subjects Active</div>
          </div>
          <div className="card stat-card">
            <div className="stat-icon green">✅</div>
            <div className="stat-value">{avgProgress}%</div>
            <div className="stat-label">Avg. Syllabus Completed</div>
          </div>
          <div className="card stat-card">
            <div className="stat-icon yellow">📝</div>
            <div className="stat-value">{completedTestsThisMonth}</div>
            <div className="stat-label">Tests Taken this Month</div>
          </div>
          </div>
        )}

        <div className="grid-2">
          {(activeTab === 'dashboard' || activeTab === 'syllabus-progress') && (
          <div className="card" id="syllabus-progress">
          ...
          {(activeTab === 'dashboard' || activeTab === 'activity-feed') && (
            <div className="card" id="activity-feed">
            <h2 style={{fontSize: '18px', fontWeight: '600', marginBottom: '16px'}}>Recent Activity</h2>
            <div style={{display: 'flex', flexDirection: 'column', gap: '16px', maxHeight: '400px', overflowY: 'auto', paddingRight: '8px'}}>
              {activities.length === 0 && <p style={{color: 'var(--text-muted)', fontSize: '14px'}}>No recent activity.</p>}
              {activities.map((act, idx) => (
                <div key={idx} style={{display: 'flex', gap: '12px'}}>
                  <div style={{background: '#EEF2FF', color: 'var(--primary-color)', width: '32px', height: '32px', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0}}>
                    {act.message.includes('Test') ? '📝' : '📚'}
                  </div>
                  <div>
                    <p style={{fontWeight: '500', fontSize: '14px'}}>{act.message}</p>
          ...
          {(activeTab === 'dashboard' || activeTab === 'weekly-tests') && (
            <div className="card" id="weekly-tests">
              <div className="progress-header">
                <h2 style={{fontSize: '18px', fontWeight: '600'}}>Weekly Tests</h2>
              </div>
              <p style={{color: 'var(--text-muted)', fontSize: '14px', marginBottom: '16px'}}>Schedule and track tests</p>
              
              {/* Guardian Schedule Test Form */}
              <form onSubmit={handleScheduleTest} style={{display: 'flex', gap: '8px', marginBottom: '24px', flexWrap: 'wrap', alignItems: 'center'}}>
                <select 
                  className="input-field" 
                ...
                <input 
                  ...
                  placeholder="Test Topics (e.g. Algebra)"
                  required
                />
                <button type="submit" className="btn" style={{flex: '0 0 auto', width: 'auto', padding: '10px 16px', backgroundColor: '#D97706'}}>Schedule Test</button>
              </form>

              <div style={{display: 'flex', gap: '20px', alignItems: 'flex-start', flexWrap: 'wrap'}}>
                <CalendarWidget tests={weeklyTests} />
                
                <div style={{flex: 2, minWidth: '300px', display: 'flex', flexDirection: 'column', gap: '16px', maxHeight: '500px', overflowY: 'auto', paddingRight: '8px'}}>
                      ...
                      <div style={{marginTop: '16px'}}>
                        {uploadTestId === test.id ? (
                           <form onSubmit={(e) => handleInlineUpload(e, test.id)} style={{display: 'flex', flexDirection: 'column', gap: '8px', padding: '12px', background: '#F9FAFB', borderRadius: '8px', border: '1px dashed #CBD5E1'}}>
                             ...
                             <div style={{display: 'flex', gap: '8px'}}>
                               <button type="submit" className="btn" style={{padding: '8px', fontSize: '14px'}}>Submit</button>
          ...
          {(activeTab === 'dashboard' || activeTab === 'test-reports') && (
            <div className="card" id="test-reports" style={{gridColumn: '1 / -1'}}>
              <div className="progress-header">
                <h2 style={{fontSize: '18px', fontWeight: '600'}}>Test Reports Gallery</h2>
              </div>
          ...
          {(activeTab === 'dashboard' || activeTab === 'teacher-remarks') && (
            <div className="card" id="teacher-remarks" style={{gridColumn: '1 / -1'}}>
              <div className="progress-header">
                <h2 style={{fontSize: '18px', fontWeight: '600'}}>Teacher Remarks</h2>
              </div>
              <p style={{color: 'var(--text-muted)', fontSize: '14px', marginBottom: '24px'}}>Weekly progress updates and comments from the teacher.</p>
